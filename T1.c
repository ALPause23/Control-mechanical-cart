...
#define PB0   (1<<0); // стрелка вперёд движения
#define PB1   (1<<1); // стрелка  назад движения
#define PB3   (1<<3); // PWM
...
void ledArrowControl(char dir, bool onOff)
{
    switch(dir)
    {
        case 'u': if (onOff) PORTB |= PB0;
                else PORTB &= ~((1<<0)|(1<<1));
                break;
        case 'd': if (onOff) PORTB |= PB1;
                else PORTB &= ~((1<<0)|(1<<1));
                break;
    }
}
