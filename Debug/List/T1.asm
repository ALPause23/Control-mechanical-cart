...
; 0000 0027     DDRB=(1<<DDB7) | (1<<DDB6) | (1<<DDB5) | (1<<DDB4) | (1<<DDB3) | (0<<DDB2) | (0<<DDB1) | (0<<DDB0);
	LDI  R30,LOW(248)
	OUT  0x17,R30
...
; 0000 002D     DDRC=(0<<DDC6) | (0<<DDC5) | (0<<DDC4) | (1<<DDC3) | (0<<DDC2) | (0<<DDC1) | (0<<DDC0);
	LDI  R30,LOW(8)
	OUT  0x14,R30
...
; 0000 0033     DDRD=(1<<DDD7) | (1<<DDD6) | (1<<DDD5) | (1<<DDD4) | (0<<DDD3) | (0<<DDD2) | (1<<DDD1) | (1<<DDD0);
	LDI  R30,LOW(243)
	OUT  0x11,R30
...
; 0000 0062     // INT0: Off
; 0000 0063     // INT1: Off
; 0000 0064     GICR|=(1<<INT1) | (1<<INT0);
	IN   R30,0x3B
	ORI  R30,LOW(0xC0)
	OUT  0x3B,R30
; 0000 0065     MCUCR=(0<<ISC11) | (1<<ISC10) | (1<<ISC01) | (0<<ISC00);
	LDI  R30,LOW(6)
	OUT  0x35,R30
; 0000 0066     GIFR=(0<<INTF1) | (0<<INTF0);
	LDI  R30,LOW(0)
	OUT  0x3A,R30
; 0000 0067 
...
; 0000 0084     ASSR = 0x00;
	RCALL SUBOPT_0x0
; 0000 0085     TCCR2 = 0x00;    // таймер отключён
...
; .FSTART _checkPortPC
; 0000 008C 
; 0000 008D     int j = 0, k = 0;
; 0000 008E 
; 0000 008F     for(i; i <= n; i++)
	ST   -Y,R27
	ST   -Y,R26
	RCALL __SAVELOCR4
;	i -> Y+6
...
; 0000 009C         return j;
	...
	RJMP _0x2000001
; 0000 009D     }
; 0000 009E     else {PORTD |= PD4;
_0x7:
	SBI  0x12,4
; 0000 009F        PORTD &= ~PD4;return 255;}
	CBI  0x12,4
	LDI  R30,LOW(255)
	LDI  R31,HIGH(255)
; 0000 00A0 }
...
; 0000 00A7         OCR2 = 0x66;
	LDI  R30,LOW(102)
	OUT  0x23,R30
; 0000 00A8         TCCR2 = 0b01101100; //start timer
	LDI  R30,LOW(108)
	OUT  0x25,R30
; 0000 00A9         while(OCR2 != 0xFF)
_0xA:
	IN   R30,0x23
	CPI  R30,LOW(0xFF)
	BREQ _0xC
...
; 0000 00B2         OCR2 = 0xFF;
	LDI  R30,LOW(255)
	OUT  0x23,R30
; 0000 00B3         TCCR2 = 0b01101100; //start timer
	LDI  R30,LOW(108)
	OUT  0x25,R30
; 0000 00B4         while(OCR2 != 0x66)
_0xE:
	IN   R30,0x23
	CPI  R30,LOW(0x66)
	BREQ _0x10
...
; 0000 00BA     }
_0xD:
; 0000 00BB     TCCR2 = 0x00; //stop timer
	LDI  R30,LOW(0)
	OUT  0x25,R30
; 0000 00BC     OCR2 = 0x00;
	OUT  0x23,R30
; 0000 00BD     if(PWMDir) { PORTB |= PB3;}
	TST  R5
	BREQ _0x11
	SBI  0x18,3
; 0000 00BE     else PORTB &= ~PB3;
	RJMP _0x12
_0x11:
	CBI  0x18,3
; 0000 00BF     return;
...
_0x14:
	RCALL _PWM
; 0000 00C7    if(statusMotor == 0)
	TST  R4
	BRNE _0x15
; 0000 00C8    statusMotor = 1;
	LDI  R30,LOW(1)
	MOV  R4,R30
; 0000 00C9    else statusMotor = 0;
	RJMP _0x16
_0x15:
	CLR  R4
; 0000 00CA }
...
; 0000 00CE     #asm("cli")
	cli
; 0000 00CF     initializationDefolt();
...
_0x17:
; 0000 00D3     {
; 0000 00D4        if(PINC & (1<<4))
	SBIS 0x13,4
	RJMP _0x1A
; 0000 00D5        {
; 0000 00D6             PORTB |= PB7;
	SBI  0x18,7
; 0000 00D7             tampUpStop = false;
	CLR  R9
; 0000 00D8             tampDownStop = false;
	CLR  R8
; 0000 00D9             PORTB &= ~PB3;
	CBI  0x18,3
...
; .FSTART _exterInt0
	RCALL SUBOPT_0x3
; 0000 00E3     if (statusMotor)
	BREQ _0x1D
; 0000 00E4     {
; 0000 00E5        switch(checkPortPC(2, 3))
	LDI  R30,LOW(2)
	LDI  R31,HIGH(2)
	ST   -Y,R31
	ST   -Y,R30
...
; 0000 00EE                PORTB |= PB6;
	SBI  0x18,6
; 0000 00EF                statusMotor = 0;
	CLR  R4
; 0000 00F0                PWMDir = false;
	CLR  R5
; 0000 00F1                tampDownStop = true;
	LDI  R30,LOW(1)
	MOV  R8,R30
; 0000 00F2                PORTB &= ~PB3;
	CBI  0x18,3
; 0000 00F3 
; 0000 00F4                break;
; 0000 00F5            default: break;
_0x23:
; 0000 00F6        }
_0x20:
; 0000 00F7 
; 0000 00F8     }
; 0000 00F9     else return;
	RJMP _0x24
_0x1D:
	RJMP _0x30
; 0000 00FA 
...
; .FSTART _exterInt1
	RCALL SUBOPT_0x3
; 0000 00FF 
; 0000 0100         if(!statusMotor) {numButtom = checkPortPC(0, 1);}
	BRNE _0x25
	LDI  R30,LOW(0)
	LDI  R31,HIGH(0)
	ST   -Y,R31
...
; 0000 0104             case 0:
	...
	BRNE _0x29
; 0000 0105 
; 0000 0106                 if(tampUpStop) return;
	TST  R9
	BRNE _0x30
; 0000 0107                 else
; 0000 0108                 {
; 0000 0109                     PORTD |= PD7;
	SBI  0x12,7
; 0000 010A                     tampDownStop = false;
	CLR  R8
; 0000 010B                     PORTB &= ~PB6;
	CBI  0x18,6
; 0000 010C                     goUpDown(1);
	LDI  R26,LOW(1)
	RCALL _goUpDown
; 0000 010D                     PORTD &= ~PD7;
	CBI  0x12,7
...
_0x29:
	...
	BRNE _0x2F
; 0000 0112 
; 0000 0113                 if(tampDownStop) return;
	TST  R8
	BRNE _0x30
; 0000 0114                 else
; 0000 0115                 {
; 0000 0116                     tampUpStop = false;
	CLR  R9
; 0000 0117                     PORTB &= ~PB5;
	CBI  0x18,5
; 0000 0118                     goUpDown(0);
	LDI  R26,LOW(0)
	RCALL _goUpDown
; 0000 0119                 }
; 0000 011A                 break;
	RJMP _0x28
; 0000 011B 
; 0000 011C             default:{PORTD |= PD6;PORTD &= ~PD6;break;}
_0x2F:
	SBI  0x12,6
	CBI  0x12,6
; 0000 011D         }
